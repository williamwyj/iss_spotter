 ...
 */ 
 const nextISSTimesForMyLocation = function(callback) {
  fetchMyIP((error, ip) => {
    if (error) {
      return callback(`Fetch IP didn't work! ${error}`, null);
    } 
    fetchCoordsByIP(ip, (error, coordinates) => {
      if (error) {
        return callback(`Fetch coordinates didn't work!, ${error}`, null);
      }
      fetchISSFlyOverTimes(coordinates, (error, passTimes) => {
        if (error) {
          return (`Fetch ISS Fly Over time didn't work! ${error}`, null);
        }
